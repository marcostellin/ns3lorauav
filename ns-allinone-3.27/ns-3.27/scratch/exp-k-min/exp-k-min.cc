...
NS_LOG_COMPONENT_DEFINE("exp-k-min");

std::string filename;
std::string path = "./scratch/exp-k-min/stats/";
...
WriteNodeAvgStats () 
{
  ...
  ss << path << "lora-stats-avg.out.txt";

  bool exist = is_file_exist (ss.str());

  file.open (ss.str(), std::fstream::app);

  if ( !exist )
    file << "psr,delayGw,delayBs" << std::endl;
  ...
  file  << (double)totRecv/(double)totSent << "," << totTimeLora.GetMilliSeconds()/(double)totRecvTimeLora << "," <<
...
WriteMeshPdr () 
{
	std::ofstream file;
  std::stringstream ss;
  ss << path << "mesh-pdr-stats.out.txt";
  bool exist = is_file_exist (ss.str());
  ...
  file << (double)nBsRecv / (double)(nBsRecv + nBsNotRecv) << std::endl;
...
WriteCoverageStats ()
{
	std::ofstream file;
  std::stringstream ss;
  ss << path << "coverage-stats.out.txt";
  file.open (ss.str());
...
WriteAvgCoverageStats ()
{
  std::ofstream file;
  std::stringstream ss;
  ss << path << "coverage-avg-stats.out.txt";
  bool exist = is_file_exist (ss.str());
...
WriteConnectivityToBsStats ()
{
	std::ofstream file;
  std::stringstream ss;
  ss << path << "connectivity-bs-stats.out.txt";
  file.open (ss.str());
...
WriteAvgConnectivityToBsStats ()
{
  std::ofstream file;
  std::stringstream ss;
  ss << path << "bs-disconnection-avg-stats.out.txt";
  bool exist = is_file_exist (ss.str());

  file.open (ss.str(), std::fstream::app);

  if ( !exist )
    file << "disctime" << std::endl;
...
main (int argc, char* argv[]){
  ...
  *****************************/
  ...
  //LogComponentEnable ("SystemExpAll", LOG_LOGIC);
  ...
  ***********************/
  ...
  cmd.AddValue ("Kata", "Minimum value of Kata", k_min);
  cmd.Parse (argc, argv);
  ...
  ************************/
  ...
  mobility.SetMobilityModel ("ns3::VirtualSprings2dMobilityModel", 
                                    ...
                                    "kAta", DoubleValue (k_min),  //5
                                    //"PredictionMode", IntegerValue (0),
                                    "KMode", IntegerValue (1),

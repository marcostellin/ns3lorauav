...
if __name__ == '__main__':
  ...
  for numUavs in numUavsList:
    ...
    for run in range(int(runs)):
      arguments = {'Time':time, 'NumTeams':numTeams, 'NumUAVs':numUavs, 'NumMembers':numMembers, 'Size':size, 'Run':run}
      command = "../../waf"
    ...
    with open('./stats/lora-stats-avg.out.txt', mode='r') as csv_file:
      ...
      if not exist:
        file.write ('numUavs,psr,delayGw,delayBs\n')

      file.write (str(numUavs) + ',' + str(psr/nRows) + ',' + str(delayGw / nRows) + ',' + str(delayBs / nRows) + '\n')
    ...
    with open('./stats/mesh-pdr-stats.out.txt', mode='r') as csv_file:
      ...
      if not exist:
        file.write ('numUavs,psr\n')

      file.write (str(numUavs) + ',' + str(psr/nRows) + '\n')
    ...
    with open ('./stats/coverage-avg-stats.out.txt', mode='r') as csv_file:
      ...
      if not exist:
        file.write ('numUavs,covered\n')

      file.write (str(numUavs) + ',' + str(covered/nRows) + '\n')
    ...
    with open ('./stats/bs-disconnection-avg-stats.out.txt', mode='r') as csv_file:
      ...
      if not exist:
        file.write ('numUavs,disctime\n')

      file.write (str(numUavs) + ',' + str(disctime/nRows) + '\n')

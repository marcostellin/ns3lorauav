...
main (int argc, char* argv[]){
  ...
  ************************/
  ...
  UdpEchoServerHelper udpEchoHelper = UdpEchoServerHelper (80);
  apps = udpEchoHelper.Install(bsNodes);

  for (uint32_t h = 0; h < ataNodes.GetN(); h++)
  {
    //Ptr<V4Ping> pinger = DynamicCast<V4Ping>(apps.Get(h));
    Ptr<Node> node = ataNodes.Get(h);
    ...
    Ptr<VirtualSprings2dMobilityModel> model = node -> GetObject<VirtualSprings2dMobilityModel>();
    model -> AddSocket (source);
    // model -> AddRemote (i.GetAddress(0));
  }
